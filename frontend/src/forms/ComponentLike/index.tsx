...
class ComponentLikeFormRaw extends React.PureComponent<Props, State> {
  ...
  public render() {
    ...
    const isEidt = !!_initialValues.name;
    return (
      <Form
        ...
        render={({ handleSubmit, form, submitting, pristine, dirty }: RenderProps) => (
          <form onSubmit={handleSubmit} className={classes.root} id="component-form">
            ...
            <Box mt={2}>
              <KPanel
                content={
                  <>
                    {this.renderTabs()}
                    <Box p={2}>{this.renderTabDetails(isEidt)}</Box>
                  </>
                }
              />
            </Box>
            ...
            {this.renderDeployButton(isEidt)}
          </form>
        )}
      />
    );
