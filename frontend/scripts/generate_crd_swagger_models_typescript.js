...
const fixGeneratedModelFile = fileName => {
  ...
  if (content.indexOf(`'metadata'?: object;`) !== -1) {
    content = content.replace("'metadata'?: object;", "'metadata'?: V1ObjectMeta;");
  ...
  if (match) {
    ...
    content = content.replace(new RegExp(match[1] + "\\.(.*)?Enum", "g"), "$1Enum");
  ...
  fs.writeFileSync(fileName, content.replace("import { RequestFile } from '../api';\n", ""));
};
...
const run = async () => {
  ...
  const currentModelsDir = __dirname + "/../../frontend/src/kappModel";
  ...
  fs.readdirSync(modelDir)
    ...
    .forEach(fileName => {
      ...
      const exportStatement = `export { ${fileName.charAt(0).toUpperCase() +
        fileName.slice(1).replace(".ts", "")} } from "./${fileName.replace(".ts", "")}";\n`;
